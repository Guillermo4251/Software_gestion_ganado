...
// ------------------------------- Años disponibles  -------------------------------
$anios_result = $conexion->query("SELECT DISTINCT YEAR(fecha) as anio FROM monta_porcino ORDER BY anio DESC");
if ($anios_result) {
...
if (!empty($anio_seleccionado) && !empty($mes_seleccionado)) {
    $tabla_parto = 'monta_porcino';
    $alias_tabla = 'mp';
    $campos = "
        mp.id_monta AS id_inseminacion, 
        g.nombre AS nombre_cerdo, 
        mp.fecha, 
        mp.raza_verraco AS padre, 
        mp.est_parto
    ";
    $join_condicion = "g.ID_animal = mp.id_cerdo";

    // ------------------------------- Consulta SQL -------------------------------
    $consultaTabla = "
        SELECT 
            $campos, 
            DATE_FORMAT(mp.fecha, '%e de %M de %Y') AS fecha_formateada, 
            DATE_FORMAT(mp.est_parto, '%e de %M de %Y') AS prox_parto, 
            MONTH(mp.fecha) AS mes, 
            YEAR(mp.fecha) AS anio
        FROM $tabla_parto AS mp
        LEFT JOIN ganado g ON $join_condicion
        WHERE MONTH(mp.fecha) = '$mes_seleccionado' 
          AND YEAR(mp.fecha) = '$anio_seleccionado'
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody id="tablaPartos">
                                            ...
                                            <tr>
                                                ...
                                                <td><?php echo !empty($fila['nombre_cerdo']) ? $fila['nombre_cerdo'] : 'No especificado'; ?></td>
                                                <td><?php echo $fila['padre']; ?></td>
                                                <td><?php echo $fila['id_inseminacion']; ?></td>
                                                <td><?php echo $fila['prox_parto']; ?></td>
                                            </tr>
                                            <?php endforeach; ?>
                                            <?php if (empty($datosTabla)): ?>
                                            <tr>
                                                <td colspan="5" class="text-center">No se encontraron datos para los criterios seleccionados.</td>
                                            </tr>
                                            <?php endif; ?>
                                        </tbody>
                                    </table>
                        ...
                        <div class="card card-body">
                            <div class="horz-grid">
                                <div class="row show-grid">
                                    <iframe width="935" height="702" src="https://lookerstudio.google.com/embed/reporting/cc2595c8-eb5c-4c0b-af58-8c54ffe1295c/page/8LBWE" frameborder="0" style="border:0" allowfullscreen sandbox="allow-storage-access-by-user-activation allow-scripts allow-same-origin allow-popups allow-popups-to-escape-sandbox"></iframe>
                                </div>
                            </div>
                        </div>                        
                    </div>
